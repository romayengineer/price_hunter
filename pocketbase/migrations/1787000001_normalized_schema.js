// Replaces the captures + per-capture products collections with the normalized
...
//
// The old `captures`/`products` collections are dropped: the scraper now
// persists through the Record API against the normalized collections only.
//
// Rules:
...
migrate(
  (app) => {
    const oldProducts = app.findCollectionByNameOrId("products");
    if (oldProducts) {
      app.delete(oldProducts);
    }
    const oldCaptures = app.findCollectionByNameOrId("captures");
    if (oldCaptures) {
      app.delete(oldCaptures);
    }
