...
/// when `product_id` is set (authoritative), otherwise the raw
/// `brand_name` captured during scraping (when it matches the brand table),
/// otherwise the best fuzzy brand match against the brand table on the full
/// name. PocketBase reports unset relations as `""`, so an empty `product_id`
/// is treated as no link (falls through to fuzzy).
fn resolve_brand<'a>(
    pp: &ProviderProductRow,
    ...
    Ok(provider_products
        ...
        .filter(|pp| {
            let combined = format!(
                "{} {}",
                pp.name,
                pp.brand_name.as_deref().unwrap_or("")
            );
            known_brands
                .iter()
                .all(|brand| brand_coverage(&combined, brand) < 1.0)
        })
        .collect())
}
