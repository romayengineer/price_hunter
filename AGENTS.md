...
## Workspace (monorepo)
This dir is the Cargo workspace root (`Cargo.toml` virtual manifest,
`members = ["crates/*"]`). Rust crates live under `crates/`:
`crates/price_hunter` (binary + infrastructure, depends on core via
`{ path = "../price_hunter_core" }`) and `crates/price_hunter_core`
(pure library). `pocketbase/`, `.opencode/`, `captures/`, `profiles/`,
`Cargo.lock` and `target/` stay at the root. Run workspace-wide
commands from here (`cargo test --workspace`,
`cargo clippy --workspace --all-targets`); binary commands need
`-p price_hunter` (e.g. `cargo run -p price_hunter -p price_hunter -- -matrix-server`).

```
price_hunter/                # monorepo + workspace root (Cargo.toml)
├── crates/price_hunter/     # binary + infrastructure (src/, tests/{live,diagnostics})
├── crates/price_hunter_core/ # pure library (src/, tests/{application,sites,fixtures})
├── pocketbase/              # migrations + scripts (JS, no SQL)
└── price_hunter_ui@../      # Flutter app lives in the sibling dir, not in this repo
```
...
- `src/main.rs` — `cargo run -p price_hunter`: opens a real, user-controlled browser and polls
...
- `src/instance.rs` — single-instance lock via a PID file at
  ...
  `cargo run -p price_hunter` kills the previous running instance (and any orphaned
...
- `pocketbase/` — PocketBase migrations (JS, no SQL) + `scripts/setup_pocketbase.sh`
  ...
  run the scraper.
- `crates/price_hunter/tests/` — live browser tests (`#[ignore]`) and
  diagnostic tools; `crates/price_hunter_core/tests/` — offline fixture-based
  integration tests (`sites/` + `fixtures/`) and use-case tests (`application/`).
...
- `Config::ensure_template()` writes a commented template on first `cargo run -p price_hunter`
...
- Sessions persist across runs: Chrome launches with a dedicated profile in
  ...
  later `cargo run -p price_hunter`s reuse the session.
...
- Any command can be made non-interactive by adding `-yes` (or `-y`): it
  ...
  `-delete-unbranded` prompts, so e.g. `cargo run -p price_hunter -- -delete-unbranded -yes`
  deletes every page without asking. The flag is a modifier — it never selects
  a command by itself.
- `cargo run -p price_hunter` opens Chrome and keeps it open until the window is closed or Ctrl+C.
...
- Optional URL arg: `cargo run -p price_hunter -- https://example.com`. Failed navigation warns
...
- `cargo run -p price_hunter -- -import-brands <file.csv>` imports the canonical brand list
...
- `cargo run -p price_hunter -- -import-unmatched` proposes canonical `products` rows from
...
- `cargo run -p price_hunter -- -match-products` scores every (provider product × canonical
  ...
  are 0.
- `cargo run -p price_hunter -- -link-matches` re-links provider products to canonical products
  ...
  matrix empty.
- `cargo run -p price_hunter -- -match-brands` assigns a brand to every provider product and
...
- `cargo run -p price_hunter -- -report-missing-brands` lists every provider product linked to a
...
- `cargo run -p price_hunter -- -delete-unbranded` lists every provider product whose name
  ...
  safety net.
- `cargo run -p price_hunter -- -matrix-server` serves the product × provider price matrix for
  ...
  `cargo run -p price_hunter`.
- `cargo run -p price_hunter -- -export-matrix <file.csv>` writes the same matrix to a CSV
...
- `cargo run -p price_hunter -- -export-products <file.csv>` writes the canonical `products`
...
- `cargo run -p price_hunter -- -export-brands <file.csv>` writes the canonical `brand` table
...
- `cargo run -p price_hunter -- -auto-scrape <url>` automatically scrapes a listing page and
  ...
  **newly-seen products** each growth (`save_incremental`, diffed by name in a
  ...
  single-instance lock as `cargo run -p price_hunter`, so a running browser session is killed
...
- Run it: start PocketBase, then `cargo run -p price_hunter -- -matrix-server`, then
...
## Tests
- `cargo test --workspace` runs all offline tests (no network/browser needed).
  Always run `cargo test --workspace` AND
  `cargo clippy --workspace --all-targets` after changing code.
- Integration tests are split by crate:
  - `crates/price_hunter_core/tests/application/` — offline use-case tests that drive `application`
    ...
    `tests/application.rs`, run with `cargo test -p price_hunter_core --test application`.
  - `crates/price_hunter_core/tests/sites/<site>.rs` — offline fixture tests (aggregated by `tests/sites.rs`);
    run all with `cargo test -p price_hunter_core` or one site with
    `cargo test -p price_hunter_core --test sites <site>`.
  - `crates/price_hunter/tests/live/` — `#[ignore]`d tests that need network, a browser session or
    PocketBase (aggregated by `tests/live.rs`; run with
    `cargo test -p price_hunter --test live -- --ignored`).
  - `crates/price_hunter/tests/diagnostics/` — env-gated debugging tools (aggregated by
    `tests/diagnostics.rs`; no-ops without their env var).
  - `crates/price_hunter_core/tests/common/mod.rs` — shared assertion helpers;
    `crates/price_hunter_core/tests/fixtures/<site>.html` — saved page dumps.
- Fixture tests share `crates/price_hunter_core/tests/common/mod.rs`
...
- Live integration tests are `#[ignore]`d and require network plus a real session:
  - `cargo test -p price_hunter --test live compreahora -- --ignored` opens Chrome using the
  ...
  - `cargo test -p price_hunter --test live beauty24 -- --ignored` and
    `cargo test -p price_hunter --test live fabilu -- --ignored` fetch the site over plain HTTP.
  - `cargo test -p price_hunter --test live parfumerie -- --ignored` opens Chrome, visits the
    `/fragancias` category, scrolls to trigger infinite scroll, and asserts at
    least 10 products with names + prices.
  - `cargo test -p price_hunter --test live store -- --ignored` requires a running PocketBase
...
- Diagnostic tools (keep these; they are no-ops without their env var):
  - `PRICE_HUNTER_PROBE_FIXTURE=<path> cargo test -p price_hunter --test diagnostics probe -- --nocapture`
    dumps the full `Detection` (container + products) for any HTML file.
  - `PRICE_HUNTER_MEASURE_FIXTURE=<path> cargo test -p price_hunter --test diagnostics measure -- --nocapture`
    ranks every candidate container (p/d/density + selected flag) — use when the
    wrong container is picked.
  - `PRICE_HUNTER_LIVE_URL=<url> PRICE_HUNTER_DUMP_HTML=1 cargo test -p price_hunter --test diagnostics live_probe -- --ignored -- --nocapture`
    opens Chrome, navigates, scrolls, optionally dumps the rendered HTML, and
    prints the detected container + products.
- Close any `cargo run -p price_hunter` browser first — Chrome locks `profiles/chrome` while
  running. A new `cargo run -p price_hunter` now kills the previous instance automatically via
  the PID file (`crates/price_hunter/src/infrastructure/instance.rs`), so manual cleanup is normally unnecessary.
  Note: test binaries run with cwd = their crate dir, so live/diagnostic tests
  resolve relative `profiles/` and `captures/` under `crates/price_hunter/`
...
- Command: `/new-fixture-test crates/price_hunter_core/tests/fixtures/<site>.html` runs the whole flow:
  ...
  `crates/price_hunter_core/tests/sites/<site>.rs` (and register it in
  `crates/price_hunter_core/tests/sites/mod.rs`), then verify
  `cargo test --workspace` + `cargo clippy --workspace --all-targets`.

Workflow summary (details in the skill):
1. Save the site's rendered grid as `crates/price_hunter_core/tests/fixtures/<site>.html`.
...
4. Probe with `crates/price_hunter/tests/diagnostics/probe.rs`; if detection differs from the
   fixture, use `crates/price_hunter/tests/diagnostics/measure.rs` / `live_probe.rs` to diagnose and
...
- If a browser-based test "passes" instantly with no output, a leftover Chrome
  is probably holding `profiles/chrome`. `cargo run -p price_hunter` now self-heals via the PID
