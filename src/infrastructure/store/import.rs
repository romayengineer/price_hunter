...
use super::types::{
    BRANDS_COLLECTION, BrandPayload, PRODUCT_BASES_COLLECTION, ProductBasePayload,
    ProductBaseImportRow, PRODUCTS_COLLECTION, ProductImportPayload, ProductImportRow, RowOutcome,
};
...
impl Store {
    ...
    /// `product_bases` collection. Rows already present (unique on
    /// `(brand, product_name)`) are skipped; the rest are created with
    /// `active = true`. Returns the number of rows created.
    pub fn import_product_bases_csv(&self, path: &std::path::Path) -> Result<usize, Error> {
        self.import_product_bases_csv_inner(path)
            .map_err(Error::from)
    }

    fn import_product_bases_csv_inner(&self, path: &std::path::Path) -> Result<usize> {
        let mut reader = csv::Reader::from_path(path)
            .with_context(|| format!("could not read CSV at {}", path.display()))?;
        let mut created = 0usize;
        let mut skipped = 0usize;
        for result in reader.records() {
            let record =
                result.with_context(|| format!("could not parse CSV at {}", path.display()))?;
            match self.import_base_row(&record)? {
                RowOutcome::Created => created += 1,
                RowOutcome::Skipped => skipped += 1,
            }
        }
        println!("Imported {created} product bases, skipped {skipped}");
        Ok(created)
    }

    fn import_base_row(&self, record: &csv::StringRecord) -> Result<RowOutcome> {
        let brand = record.get(0).unwrap_or_default().trim().to_string();
        let product_name = record.get(1).unwrap_or_default().trim().to_string();
        if product_name.is_empty() {
            return Ok(RowOutcome::Skipped);
        }
        if brand.eq_ignore_ascii_case("brand") && product_name.eq_ignore_ascii_case("product_name") {
            return Ok(RowOutcome::Skipped);
        }
        if self.find_product_base(&brand, &product_name)?.is_some() {
            return Ok(RowOutcome::Skipped);
        }
        self.client
            ...
            .call()
            .map_err(|e| anyhow::anyhow!("could not import product base: {e}"))
            .map(|_| RowOutcome::Created)
    }

    fn find_product_base(
        &self,
        brand: &str,
        product_name: &str,
    ) -> Result<Option<ProductBaseImportRow>> {
        let filter = format!(
            "brand='{}' && product_name='{}'",
            escape_filter(brand),
