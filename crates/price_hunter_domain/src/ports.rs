...
pub trait ProductCatalog {
    ...
    /// with the same `(brand, product_name)` is already present.
    fn create_product(
        &self,
        brand: &str,
        product_name: &str,
    ...
    /// that reference it and cascade-deleting its `provider_product_matches`
    /// rows. Provider products are kept unlinked for future re-matching.
