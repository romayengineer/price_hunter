...
use price_hunter_core::domain::error::PriceStoreError;
use price_hunter_core::domain::matching::{MIN_SCORE, MatchCandidate};
use price_hunter_core::domain::model::{
    BrandRow, MatchInsert, ProductInsert, ProductRow, ProviderMatchRow, ProviderProductRow,
    ProviderRow,
};
use price_hunter_core::domain::ports::{
...
pub struct FakeStore {
    ...
    deleted_products: RefCell<Vec<String>>,
    created_products: RefCell<Vec<(String, String, String)>>,
}
...
impl FakeStore {
    ...
    /// `(brand, product_name, name)` tuples.
    pub fn created_products(&self) -> Vec<(String, String, String)> {
        self.created_products.borrow().clone()
    }

    /// The canonical products deleted via `delete_product`.
    #[allow(dead_code)]
    pub fn deleted_products(&self) -> Vec<String> {
        self.deleted_products.borrow().clone()
    }
}
...
impl ProductCatalog for FakeStore {
    fn list_products(&self) -> Result<Vec<ProductRow>, PriceStoreError> {
        self.check()?;
        let deleted = self.deleted_products.borrow();
        Ok(self
            .products
            .iter()
    ...
    fn create_product(
        ...
        let exists = self
            ...
            || self
                ...
                .borrow()
                .iter()
                .any(|(_, _, n)| n.eq_ignore_ascii_case(name));
        if exists {
    ...
    fn delete_product(&self, product_id: &str) -> Result<(), PriceStoreError> {
        self.check()?;
        // Cascade: keep provider_products unlinked, drop matches referencing product_id
        for pp in &self.provider_products {
            if pp.product_id.as_deref() == Some(product_id) {
                self.product_links
                    .borrow_mut()
                    .insert(pp.id.clone(), None);
            }
        }
        self.matches
            .borrow_mut()
            .retain(|m| m.product_id != product_id);
    ...
    fn list_provider_products(&self) -> Result<Vec<ProviderProductRow>, PriceStoreError> {
        ...
        Ok(self
            ...
            .map(|p| ProviderProductRow {
                ...
                name: p.name.clone(),
                provider_product_url: p.provider_product_url.clone(),
